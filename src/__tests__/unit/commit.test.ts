...
describe("commit", () => {
  ...
  describe("createTree", () => {
    ...
    it("should chain createTree 3 times for 201 files (2 full chunks + 1)", async () => {
      ...
      expect(mockOctokit.rest.git.createTree.mock.calls[2][0].tree).toHaveLength(
        1
      );
      expect(mockOctokit.rest.git.createBlob).not.toHaveBeenCalled();
    });
    ...
    it("should start a new chunk on byte budget before the count limit is reached", async () => {
      ...
      expect(mockOctokit.rest.git.createBlob).not.toHaveBeenCalled();
      expect(
        mockOctokit.rest.git.createTree.mock.calls[0][0].tree
      ).toHaveLength(2);
      expect(mockOctokit.rest.git.createTree.mock.calls[0][0].base_tree).toBe(
        "base-tree-sha"
      );
      expect(
        mockOctokit.rest.git.createTree.mock.calls[1][0].tree
      ).toHaveLength(2);
      expect(mockOctokit.rest.git.createTree.mock.calls[1][0].base_tree).toBe(
        "byte-chunk-1"
      );

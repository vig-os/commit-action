...
import { calculateDelay, classifyError, isTransientError, withRetry } from '../../retry.js';
...
describe('retry', () => {
  describe('isTransientError', () => {
    ...
    it("returns true for status 403 with message containing 'secondary rate limit'", () => {
      expect(isTransientError({ status: 403, message: 'secondary rate limit exceeded' })).toBe(
        true
      );
      expect(isTransientError({ status: 403, message: 'API rate limit exceeded' })).toBe(true);
      expect(isTransientError({ status: 403, message: 'You have exceeded abuse rate limit' })).toBe(
        true
      );
    });
  ...
  describe('classifyError', () => {
    ...
    it('returns descriptive string for 403 rate limit', () => {
      expect(classifyError({ status: 403, message: 'secondary rate limit' })).toMatch(
        /rate limit|403/i
      );
    });
